 ...
 *
 * some modification
 *         - added workaround to allow CTRL-A
 *         - added workaround to keep all text selected when "TAB"-ing from previous fields
 *         - move the cursor appropriately when first text is [decimal] or -[decimal]
 *         - prioritize o.setSelectionRange over o.createTextRange
 */
(function($) {
/*
...
$.fn.numeric.keyup = function(e)
{
	...
	if(key == 17) { // allow Ctrl
		$(this).data("_previous.key", key);
		return false; 
	} else if( $.data(this, "_previous.key") === 17 && key === 65 ) { // allow Ctrl+A
		$(this).data("_previous.key", null);
		return false; 
	} else if(key == 9) {  // ignore tab from previous field.
		return false;
	} else {
		$(this).data("_previous.key", null);
	}
...
{	
	...
	} else if (o.createTextRange && document.selection && document.selection.duplicate)
	{
		var r = document.selection.duplicate();
